---
title: Upload Objects (Tus Protocol) | BitPaket Docs
description: Upload objects to BitPaket using Tus Protocol
service: api
main: ''
author: gencer
icon: browser-upload
...
We will tell you some key features that we implemented in our Tus endpoint. Please see below how to use Tus protocol. If you follow tus protocol, you will successfully upload your objects to our service.

{:toc}

## Endpoint

### Request Upload ID (POST)
...
Upload-Metadata: filename bXlmaWxlLnppcA==,filetype aW1hZ2UvanBlZw==, X-Bit-Versioning MQ==, X-Bit-Folder L2Rpci9mb2xkZXI=
```
...
Access-Control-Expose-Headers:	Authorization, Upload-Offset, Location, Upload-Length, Tus-Version, Tus-Resumable, Tus-Max-Size, Tus-Extension, Upload-Metadata, Upload-Checksum, Upload-Defer-Length
Location:	https://api.bitpaket.com/v2/upload/723a190c-df46-4abe-b354-87f9b90b9f24
Tus-Resumable:	1.0.0
```
```json
{
...
```http
PATCH /v2/upload/{upload-id} HTTP/1.1
Host: api.bitpaket.com
Content-Type: application/json; charset=utf-8
...
> **Note:** Authorization on `PATCH` requests is not required. Due to you are already authenticated on `POST` and get your `Upload ID`, we will save your authorization for this `Upload ID`
> 
> **Keep in mind that**, Upload ID is your **Object ID** and *won't change* after you finish your upload.
...
At tus protocol, currently authentication can be a problematic. Due to this, BitPaket implemented its own authentication middleware. Each POST request should include `Authorization` header. Otherwise authentication will fail and you won't get Upload ID.

## Upload ID and Object ID

Current Tus implementations are using SHA1 or similar upload IDs. We at BitPaket merged Upload ID with Object ID. Once you get your Upload ID, You also get your Object ID.

Object IDs (Upload ID) is UUIDv4.

## Metadata

Tus protocol supports metadata. Please send your `X-Bit-*` headers in Tus metadata object. *See above example.*

## Response

Currently Tus protocol also does not return any BODY when Upload ID Requested.

When you do POST request and grab your Upload ID, We also send you the metadata simulation. You can refer this metadata for further reference.

## Getting metadata of an uploaded (finished) object

After you finish your upload, you can get your metadata by using our [metadata endpoint](/getting-metadata) using your Object (Upload) ID.

Below, you can find a copy of [tus.io/protocol](https://tus.io/protocols/resumable-upload.html)
